...
|---|---|---|---|---|
| GET | `/tours` | public | query: `page,limit,destinationId?,minPrice?,maxPrice?,sort?` | paginated **Tour** (APPROVED only) |
...
`sort` accepts `price` \| `-price` \| `title`. Public listing/detail only ever returns `APPROVED`
tours; the DRAFT→PENDING_REVIEW→APPROVED lifecycle is operator/admin-facing.
...
UI tabs). See §8-adjacent lifecycle below.
...
| Method | Path | Auth | Body | `data` |
|---|---|---|---|---|
| POST | `/uploads/image` | OPERATOR or ADMIN | `multipart/form-data`, field `file` (image) | `{ url, publicId }` |

Returns the hosted Cloudinary URL. `400` if the file isn't an image; `503` if Cloudinary isn't
configured on the server. **See §9 for the important role caveat.**
...
1. **Avatar upload role mismatch.** `PATCH /users/me` accepts `avatarUrl`, and the intended flow is
   "upload an image, then pass its URL." But `POST /uploads/image` currently requires **OPERATOR or
   ADMIN** — a **TOURIST cannot upload via that endpoint (they get `403`)**. Until the backend opens
   uploads to tourists, a tourist avatar must come from an **externally hosted URL**. Confirm the
   intended approach with the backend team.
